<template>
        ...
        <template v-slot:bottom>
          <div style="width:100%;" class="flex justify-end">
            ...
            <q-input
              ...
              disable
              dense
              input-style="width:30px; font-size: 15px;"
  ...
  <WorkUnitUploadVue
    :inception="workModal"
    :companies="companies"
    @close="workModal = false"
  />
  <WorkScheduleDetailModal
    :inception="workDetailModal"
    :workDetail="detail"
    @close="workDetailModal=false"
  />>
</template>

<script>
import { ref } from "vue";
import FileUnitUploadVue from "../modal/FileUnitUploadModal.vue";
import WorkUnitUploadVue from "../modal/WorkUnitUploadModal.vue";
import WorkScheduleDetailModal from "../modal/WorkScheduleDetailModal.vue";
...
const data = [
  ...
  },
  {
    ...
    insure_name: "한화생명",
    계정: 20139068,
    work_kind: "신계약 자료 다운로드",
    filename: "NEW_LIST_ALL_C(건별계약상세)",
    schedule: "오전 2시",
    timeout: 0,
    lock: false,
    ERP연동: ".",
...
export default {
  name: "TableRPA",
  components: { FileUnitUploadVue, WorkUnitUploadVue,WorkScheduleDetailModal },
  setup() {
    const show_filter = ref(false);
    const selected = ref([]);
    let fileModal = ref(false);
    let workModal = ref(false);
    let workDetailModal = ref(false);
    let detail = ref({})
    ...
    return {
      ...
      filter: ref(""),
      pagination,
      companies: ["에즈금융서비스", "마이금융파트너"],
      lockCheck: ["모두", "열림", "잠김"],
      wokr_kind: ["신계약", "수금", "수수료", "모니터링"],
      spec: ["생보", "손보"],
      insure: ["DB생명", "DB손보", "KB생명", "KB손보", "삼성생명", "삼성화재"],
      show_filter,
      data,
      columns,
      fileModal,
